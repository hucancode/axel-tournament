...
async fn main() -> Result<()> {
    ...
    let metadata = GameMetadata {
        id: GAME_ID,
        name: "Tic Tac Toe",
        description: "Classic tic-tac-toe game for 2 players - supports both automated and interactive modes",
    ...
    let http_server = tokio::spawn(async move {
        if let Err(e) = run_http_server::<TicTacToeGame>(SERVER_PORT, metadata).await {
    ...
    let match_watcher = tokio::spawn(async move {
        let config = MatchWatcherConfig {
            ...
            game_config: GameConfig {
                num_rounds: 1, // Tic-tac-toe is a single game
                turn_timeout_ms: 30000,
                memory_limit_mb: 64,
                cpu_limit: 0.5,
            },
        ...
        if let Err(e) = run_match_watcher::<TicTacToeGame>(config).await {
