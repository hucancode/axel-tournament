import { api } from '../api';
import type { Match, CreateMatchRequest, UpdateMatchResultRequest } from '../types';

export const matchService = {
	async list(filters?: { tournament_id?: string; game_id?: string; user_id?: string }): Promise<Match[]> {
	...
	async create(data: CreateMatchRequest): Promise<Match> {
		return api.post<Match, CreateMatchRequest>('/api/admin/matches', data, true);
	},

	async updateResult(id: string, data: UpdateMatchResultRequest): Promise<Match> {
