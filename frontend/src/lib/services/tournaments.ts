import { api } from '../api';
import type { Tournament, TournamentParticipant, CreateTournamentRequest } from '../types';
...
export const tournamentService = {
	...
	async getParticipants(id: string): Promise<TournamentParticipant[]> {
		return api.get<TournamentParticipant[]>(`/api/tournaments/${id}/participants`);
	},
	...
	async create(data: CreateTournamentRequest): Promise<Tournament> {
		return api.post<Tournament, CreateTournamentRequest>('/api/admin/tournaments', data, true);
	},
