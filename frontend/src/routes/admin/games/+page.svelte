<script lang="ts">
	...
	import type { Game, ProgrammingLanguage, CreateGameRequest } from '$lib/types';
	...
	function toggleLanguage(lang: ProgrammingLanguage) {
		if (formData.supported_languages.includes(lang)) {
			formData.supported_languages = formData.supported_languages.filter(l => l !== lang);
		} else {
			formData.supported_languages = [...formData.supported_languages, lang];
		}
	}
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			formError = err instanceof Error ? err.message : 'Failed to save game';
		} finally {
			formLoading = false;
		}
	}
			...
			{#each games as game}
				<div class="card">
					<div class="flex justify-between items-start mb-2">
						...
						<span class="badge {game.is_active ? 'badge-accepted' : 'badge-cancelled'}">
					...
					<div class="mb-4">
						<h4 class="font-semibold text-sm text-gray-700 mb-2">Supported Languages:</h4>
						<div class="flex gap-2">
					...
					<div class="mb-4">
						<h4 class="font-semibold text-sm text-gray-700 mb-1">Rules:</h4>
						<pre class="text-sm text-gray-600" style="background: var(--gray-50); padding: 0.5rem; border-radius: 0.25rem; overflow-x: auto;">{JSON.stringify(game.rules, null, 2)}</pre>
					</div>

					<p class="text-sm text-gray-500 mb-4">Created: {formatDate(game.created_at)}</p>
...
{#if showForm}
	...
	>
		...
		>
			...
			{#if formError}
				<div style="background-color: #fee2e2; border-left: 4px solid var(--red-600); padding: 1rem; margin-bottom: 1rem;">
			...
			<form onsubmit={handleSubmit}>
				...
				<div class="form-group">
					...
					<p class="text-sm text-gray-500" style="margin-top: 0.25rem;">
						Enter game rules as JSON. Example: {"{"}"max_turns": 100, "grid_size": 10{"}"}
					</p>
				</div>
				...
				<div class="flex gap-2">
					<button
						...
						disabled={formLoading}
					>
						{formLoading ? 'Saving...' : isEditing ? 'Update Game' : 'Create Game'}
					</button>
					<button
						type="button"
						class="btn btn-secondary"
