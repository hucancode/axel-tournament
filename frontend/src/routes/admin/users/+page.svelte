<script lang="ts">
	...
	async function loadUsers() {
		...
		try {
			const allUsers = await adminService.listUsers(currentPage, pageSize);
			users = allUsers;
	...
	async function handleBan() {
		...
		try {
			...
			banError = err instanceof Error ? err.message : 'Failed to ban user';
		} finally {
			banLoading = false;
		}
	}
					...
					<tbody>
						{#each users as user}
							<tr>
								...
								<td>{user.email}</td>
								<td>
									<span class="badge {user.role === 'admin' ? 'badge-running' : 'badge-scheduled'}">
										{user.role}
									</span>
								</td>
...
{#if showBanDialog && selectedUser}
	...
	>
		<div
			...
			"
		>
			<h2 class="font-bold text-xl mb-4">Ban User: {selectedUser.username}</h2>
			...
			<form onsubmit={(e) => { e.preventDefault(); handleBan(); }}>
