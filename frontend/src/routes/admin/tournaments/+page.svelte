<script lang="ts">
	...
	import type { Tournament, Game, CreateTournamentRequest, TournamentStatus } from '$lib/types';
	...
	async function handleSubmit(e: Event) {
		...
		try {
			if (isEditing && editingTournament) {
				await tournamentService.update(editingTournament.id, submitData);
			} else {
			...
			formError = err instanceof Error ? err.message : 'Failed to save tournament';
		} finally {
			formLoading = false;
		}
	}
...
<div class="container page">
	...
	{:else if games.length === 0}
		<div class="card text-center">
			<p class="text-gray-500">Please create a game first before creating tournaments.</p>
	...
	{:else if tournaments.length === 0}
		<div class="card text-center">
			<p class="text-gray-500">No tournaments found. Create your first tournament!</p>
		</div>
	{:else}
		<div class="card">
				...
				<table>
					...
					<tbody>
						{#each tournaments as tournament}
							<tr>
								<td>
									<a href="/tournaments/{tournament.id}" class="font-semibold" style="color: var(--primary-600); text-decoration: none;">
										{tournament.name}
									</a>
								</td>
								...
								<td>
									...
									<span class="text-sm text-gray-500">(min: {tournament.min_players})</span>
								</td>
								<td class="text-sm">
									{tournament.start_time ? formatDate(tournament.start_time) : 'Not set'}
								</td>
								<td class="text-sm">
									{tournament.end_time ? formatDate(tournament.end_time) : 'Not set'}
								</td>
								<td>
									<button
			...
			<form onsubmit={handleSubmit}>
				...
				<div class="flex gap-2">
					<button
						...
						disabled={formLoading}
					>
						{formLoading ? 'Saving...' : isEditing ? 'Update Tournament' : 'Create Tournament'}
					</button>
					<button
						type="button"
						class="btn btn-secondary"
