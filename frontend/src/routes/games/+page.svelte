						...
						<div class="card">
							...
							<div class="mb-4">
								...
								<div class="flex gap-2">
									{#each game.supported_languages as lang}
										<span class="badge badge-scheduled">{lang.toUpperCase()}</span>
									{/each}
								</div>
							</div>
							...
							<div class="mb-4">
								...
								<div class="grid gap-2" style="grid-template-columns: auto 1fr;">
									...
									<div class="text-sm text-gray-500">Total Tournaments:</div>
									<div class="text-sm font-semibold">
							...
							{#if game.rules && Object.keys(game.rules).length > 0}
								<div class="mb-4">
									...
									<pre class="text-sm" style="background: var(--gray-100); padding: 0.75rem; border-radius: 0.5rem; overflow-x: auto; max-height: 200px;">{JSON.stringify(game.rules, null, 2)}</pre>
								</div>
							{/if}
