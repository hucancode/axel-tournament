	...
	<div class="container">
		...
		{#if !$authStore.isAuthenticated}
			<div class="card text-center" style="max-width: 600px; margin: 2rem auto;">
				...
				<p class="mb-4">Create an account to participate in tournaments and submit your code</p>
		...
		{:else}
			<div class="card" style="margin-bottom: 2rem;">
				<h2 class="text-xl font-semibold mb-2">Welcome back, {user?.username}!</h2>
				<div class="flex gap-4" style="margin-top: 1rem;">
					<a href="/tournaments" class="btn-primary">Browse Tournaments</a>
		...
		{:else}
			<div class="grid grid-2">
				{#each tournaments.slice(0, 6) as tournament}
					<a href="/tournaments/{tournament.id}" class="card" style="text-decoration: none; color: inherit; transition: transform 0.2s;" onmouseenter={(e) => e.currentTarget.style.transform = 'translateY(-4px)'} onmouseleave={(e) => e.currentTarget.style.transform = 'translateY(0)'}>
						<h3 class="text-lg font-semibold mb-2">{tournament.name}</h3>
						<p class="text-sm text-gray-700 mb-4" style="overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">
			...
			{#if tournaments.length > 6}
				<div class="text-center mt-4">
					<a href="/tournaments" class="btn-primary">View All Tournaments</a>
				</div>
			{/if}
		{/if}
	</div>
