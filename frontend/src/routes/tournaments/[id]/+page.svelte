<script lang="ts">
	...
	async function loadTournamentData() {
		...
		try {
			...
			if ($authStore.isAuthenticated && $authStore.user) {
				isParticipant = participants.some(p => p.user_id === $authStore.user!.id);
			}
	...
	async function joinTournament() {
		...
		try {
			...
			actionError = err instanceof Error ? err.message : 'Failed to join tournament';
	...
	async function leaveTournament() {
		...
		try {
			...
			actionError = err instanceof Error ? err.message : 'Failed to leave tournament';
	...
	function canLeave(): boolean {
		if (!tournament || !$authStore.isAuthenticated) return false;
		if (!isParticipant) return false;
		if (tournament.status === 'running' || tournament.status === 'completed') return false;
		return true;
	}
				...
				<div class="flex-col gap-4" style="display: flex;">
					...
					<div class="card">
						...
						<div class="grid gap-2" style="grid-template-columns: auto 1fr;">
							...
							<div class="font-semibold text-gray-700">Players:</div>
							<div>
							...
							{#if tournament.end_time}
								<div class="font-semibold text-gray-700">End Time:</div>
					...
					{#if game}
						<div class="card">
							<h2 class="text-xl font-semibold mb-4">Game: {game.name}</h2>
							<p class="text-gray-700 mb-4">{game.description}</p>

							<div class="mb-4">
								<div class="font-semibold text-gray-700 mb-2">Supported Languages:</div>
							...
							{#if game.rules && Object.keys(game.rules).length > 0}
								<div>
									<div class="font-semibold text-gray-700 mb-2">Game Rules:</div>
									<pre class="text-sm" style="background: var(--gray-100); padding: 1rem; border-radius: 0.5rem; overflow-x: auto;">{JSON.stringify(game.rules, null, 2)}</pre>
								</div>
							{/if}
						</div>
					{/if}
					...
					<!-- Participants -->
					<div class="card">
						<h2 class="text-xl font-semibold mb-4">Participants ({participants.length})</h2>
								...
								<tbody>
									{#each participants as participant}
										<tr>
											<td>
												{#if participant.rank}
													<span class="font-semibold">#{participant.rank}</span>
												{:else}
											...
											<td class="font-semibold">
												...
												{#if $authStore.user && participant.user_id === $authStore.user.id}
													<span class="badge badge-registration" style="margin-left: 0.5rem;">You</span>
												{/if}
											</td>
				...
				<div class="flex-col gap-4" style="display: flex;">
					<div class="card">
						...
						{:else}
							...
							{#if isParticipant}
								<div class="mt-4" style="padding: 1rem; background: var(--primary-50); border-radius: 0.5rem;">
									<p class="text-sm text-center font-semibold" style="color: var(--primary-700);">
					...
					{#if tournament.status === 'registration'}
						<div class="card" style="background: var(--primary-50);">
							<h3 class="font-semibold mb-2" style="color: var(--primary-700);">
								Registration Open
							</h3>
							<p class="text-sm" style="color: var(--primary-700);">
					...
					{:else if tournament.status === 'running'}
						<div class="card" style="background: #fef3c7;">
							...
							<p class="text-sm" style="color: #92400e;">
								This tournament is currently running. New participants cannot join.
							</p>
						</div>
					{:else if tournament.status === 'completed'}
						<div class="card" style="background: #d1fae5;">
							<h3 class="font-semibold mb-2" style="color: #065f46;">
