			...
			<div class="grid grid-2">
				{#each tournaments as tournament}
					<div class="card">
						...
						<div class="grid gap-2 mb-4" style="grid-template-columns: auto 1fr;">
							<div class="text-sm text-gray-500 font-semibold">Players:</div>
							<div class="text-sm">
								{tournament.current_players} / {tournament.max_players}
								{#if tournament.current_players >= tournament.max_players}
									<span class="badge badge-failed" style="margin-left: 0.5rem;">Full</span>
								{:else if tournament.current_players >= tournament.min_players}
									<span class="badge badge-accepted" style="margin-left: 0.5rem;">Active</span>
								{:else}
									<span class="badge badge-pending" style="margin-left: 0.5rem;">
										Need {tournament.min_players - tournament.current_players} more
									</span>
								{/if}
							</div>

							{#if tournament.start_time}
								<div class="text-sm text-gray-500 font-semibold">Start:</div>
								<div class="text-sm">{formatDate(tournament.start_time)}</div>
							{/if}

							{#if tournament.end_time}
								<div class="text-sm text-gray-500 font-semibold">End:</div>
								<div class="text-sm">{formatDate(tournament.end_time)}</div>
							{/if}
						</div>

						<a href="/tournaments/{tournament.id}" class="btn btn-primary" style="width: 100%;">
							View Details
						</a>
					</div>
				{/each}
