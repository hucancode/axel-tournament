use crate::{
    error::ApiResult,
    models::{Claims, CreateTournamentRequest, Tournament, TournamentParticipant, TournamentStatus, UpdateTournamentRequest},
    services,
    AppState,
};
use axum::{
...
pub async fn list_tournaments(
    ...
    let status = if let Some(status_str) = query.status {
        Some(serde_json::from_str::<TournamentStatus>(&format!("\"{}\"", status_str))
