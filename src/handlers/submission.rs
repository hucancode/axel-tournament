use crate::{
    error::ApiResult,
    models::{Claims, CreateSubmissionRequest, ProgrammingLanguage, Submission, SubmissionResponse},
    services,
    AppState,
};
use axum::{
...
pub async fn create_submission(
    ...
    let language = ProgrammingLanguage::from_str(&payload.language)
        .ok_or_else(|| crate::error::ApiError::Validation("Invalid programming language".to_string()))?;

    // Get tournament to verify it exists and get game_id
    let tournament = services::tournament::get_tournament(&state.db, &payload.tournament_id).await?;
