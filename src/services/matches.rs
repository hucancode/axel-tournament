use crate::{
    ...
    models::{
        matches::{Match, MatchParticipant, MatchStatus, MatchParticipantResult},
...
pub async fn create_match(
    ...
    for sub_id in participant_submission_ids {
        ...
        let submission = submission.ok_or_else(|| ApiError::NotFound(format!("Submission {} not found", sub_id)))?;
        ...
        if submission.game_id != game.id.clone().unwrap() {
             return Err(ApiError::BadRequest(format!("Submission {} does not belong to game {}", sub_id, game_id_clean)));
        }
    ...
    let new_match = Match {
        id: None,
        tournament_id: tournament_id.map(|id| Thing::from(("tournament", id.trim_start_matches("tournament:")))),
...
pub async fn list_matches(
    ...
    if let Some(tid) = tournament_id {
        query.push_str(&format!(" AND tournament_id = type::thing('tournament', '{}')", tid));
    }
    ...
    if let Some(uid) = user_id {
        query.push_str(&format!(" AND participants[?].user_id CONTAINS type::thing('user', '{}')", uid));
    }
...
pub async fn update_match_result(
    ...
    for res in participants_results {
        ...
        if let Some(p) = match_data.participants.iter_mut().find(|p| p.submission_id == target_thing) {
